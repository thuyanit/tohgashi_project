...
<form action="" method="post">
    <div class="group_btn">
	    <p><a href="<?php echo plugins_url($PDF); ?>" download="飲食券申込書.pdf">申請フォーマットダウンロード</a></p>
        <?php
        if($date6>=$now_date){
        ?>
        <p><a class="mailto" id="send6" key="<?php echo '飲食券申込書 - '.$company->company_name. ' <' .$location_mail.'>'; ?>" href="">申請・図面送信</a></p>
        <?php   
        }
        else{
        ?>
        <p><a class="expries" data-notice="飲食券申込書">申請・図面送信</a></p>
        <?php
        }
