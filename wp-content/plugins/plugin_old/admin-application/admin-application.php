...
function fc_list_exhibitor_admin(){
    ...
    //Download button 2 By Mr.VietSy
    $orders =  $wpdb->get_results(
        "SELECT orders.id, customer_id, product_detail, product_spec_detail, email_dk, company_name, date_dk, number_booth
         FROM orders
         LEFT JOIN order_items ON orders.id = order_items.order_id
         LEFT JOIN order_items_spec ON order_items.id = order_items_spec.order_items_id
         LEFT JOIN register_togashi ON orders.customer_id = register_togashi.id
         LEFT JOIN register_togashi_link ON register_togashi.id = register_togashi_link.togashi_id
         WHERE orders.catalog_id = ".$location." AND register_togashi_link.catalog_id = ".$location." AND status = 1"
    );
    if( $orders )
    {
        $tmp = array();
        ...
        {
            $val->product_detail             = @json_decode($val->product_detail);
            $val->product_spec_detail        = @json_decode($val->product_spec_detail);
            $tmp[$val->id]['togashi_id']     = $val->customer_id;
            $tmp[$val->id]['email_dk']       = $val->email_dk;
            $tmp[$val->id]['company_name']   = $val->company_name;
            $tmp[$val->id]['date_dk']        = $val->date_dk;
            $tmp[$val->id]['product_detail'][] = $val->product_detail;
            $tmp[$val->id]['product_spec'][] = $val->product_spec_detail;
            $tmp[$val->id]['number_booth']  = $val->number_booth;
        }

        foreach ($tmp as $key => $val)
        {
            // $togashi_link = $wpdb->get_row("SELECT * FROM register_togashi_link WHERE togashi_id = ".$val['togashi_id']." AND catalog_id = ".$location." AND status = 1");
            // if( $togashi_link ) {
            //     $val['number_booth'] = $togashi_link->number_booth;
            // }

            $exhibitor = $wpdb->get_row("SELECT * FROM exhibitor_foundation WHERE email_dk = '".$val['email_dk']."' AND location = ".$location);
            if( $exhibitor ) {
                $val['info'] = @unserialize($exhibitor->info);
            }
            $tmp[$key] =(object) $val;
        }        
    ...
    // Download 電気工事申込
//     $data = [];
//     $output1 = $wpdb->get_results($wpdb->prepare(
//     "SELECT register_togashi.company_name AS company_name,register_togashi_meta.meta_value AS META_RES1, register_togashi_link.togashi_id, exhibitor_payment.pay_tel AS pay_tel,
//     exhibitor_payment.pay_fax AS pay_fax, exhibitor_construction.info AS IN_CON, exhibitor_foundation.info AS IN_FOU
//     FROM register_togashi
//     INNER JOIN register_togashi_link ON register_togashi.id = register_togashi_link.togashi_id
//     LEFT JOIN exhibitor_construction ON register_togashi.id = exhibitor_construction.togashi_id
//     LEFT JOIN exhibitor_foundation ON register_togashi.id = exhibitor_foundation.togashi_id
//     LEFT JOIN register_togashi_meta ON register_togashi.id = register_togashi_meta.togashi_id
//     LEFT JOIN exhibitor_payment ON register_togashi.id = exhibitor_payment.register_id
//     WHERE register_togashi_link.status = 1
//     AND register_togashi_link.catalog_id = %d
//     AND exhibitor_construction.location = %d
//     AND exhibitor_foundation.location = %d
//     AND register_togashi_meta.meta_key = 'person_in_charge'
//     ",
//     $location,
//     $location,
//     $location
...
//     $output2 = $wpdb->get_results($wpdb->prepare(
//       "SELECT register_togashi_meta.meta_value AS META_RES2, register_togashi_meta.togashi_id
//       FROM register_togashi_meta
//       LEFT JOIN register_togashi ON register_togashi_meta.togashi_id = register_togashi.id
//       LEFT JOIN exhibitor_construction ON register_togashi_meta.togashi_id  = exhibitor_construction.togashi_id
//       LEFT JOIN exhibitor_foundation ON register_togashi_meta.togashi_id  = exhibitor_foundation.togashi_id
//       WHERE register_togashi_meta.meta_key = 'emergency_tel'
//       AND exhibitor_construction.location = %d
//       AND exhibitor_foundation.location = %d
//       ",
//       $location,
...
//       {
//         $output1[$key] = (array)$val;
//       }

//       foreach($output2 as $key => $val)
//       {
//         $output2[$key] = (array)$val;
//       }
//       $combined = array();
//       foreach ($output1 as $arr) {
//         $comb = array('togashi_id' => $arr['togashi_id'], 'company_name' => $arr['company_name'], 'META_RES1' => $arr['META_RES1'],
//         'pay_tel' => $arr['pay_tel'], 'pay_fax' => $arr['pay_fax'], 'IN_CON' => $arr['IN_CON'], 'IN_FOU' => $arr['IN_FOU'], 'META_RES2' => '');
//         foreach($output2 as $arr2){
//           if($arr2['togashi_id'] == $arr['togashi_id']){
//             $comb['META_RES2'] = $arr2['META_RES2'];
//           }
//         }
//         $combined[] = $comb;
//       }
...
//   {
//     $in_con = unserialize($value['IN_CON']);
//     $in_fou = unserialize($value['IN_FOU']);
//     // Check number_booth set value ....

//     switch($in_fou['qty_booth']){
//       case 1:
//           $spot_100W = '2';
//           $spot_500W = '1';
//           $primary_electric = '1';
...
//           $spot_500W = '3';
//           $primary_electric = '3';
//           $distribution_board = '3';
//           break;
//       default:
//           $spot_100W = Null;
//           $spot_500W = Null;
//           $primary_electric = Null;
//           $distribution_board = Null;
//           break;
//     }
//     $itm = [];
//     $itm = [
//       'company_name' => $value['company_name'],
//       'number_booth' => $in_fou['qty_booth'],
//       'person_in_charge' => $value['META_RES1'],
//       'tel' => $value['pay_tel'],
//       'fax' => $value['pay_fax'],
//       'emergency_phone' => $value['META_RES2'],
//       'spot_100W' => $spot_100W,
//       'spot_500W' => $spot_500W,
//       'primary_electric' => $primary_electric,
//       'distribution_board' => $distribution_board,
//       'electric_use' => $in_con['electric_use'],
//       'optional_electric1' => $in_con['optional_electric1'],
//       'optional_electric2' => $in_con['optional_electric2'],
//       'optional_electric3' => $in_con['optional_electric3'],
//       'optional_electric4' => $in_con['optional_electric4'],
//       'optional_electric5' => $in_con['optional_electric5'],
//       'optional_electric6' => $in_con['optional_electric6']
//       //TOTAL COST ELECTRIC is here
// ]   ;
//     $data[] = $itm;
// }
...
function fc_registration_dealine_admin(){
	...
	$date = $wpdb->get_row("SELECT * FROM dealine_app WHERE location = ".$location);

	if ($date) {
		$date->dealine_all = date('Y/m/d',strtotime($date->dealine_all));
		$date->dealine_billing = date('Y/m/d',strtotime($date->dealine_billing));
		$date->dealine_goods = date('Y/m/d',strtotime($date->dealine_goods));

		$update1 = strtotime($_POST['dealine_all']);
        $dealine_all=  date("Y/m/d", $update1);

        $update2 = strtotime($_POST['dealine_billing']);
        $dealine_billing=  date("Y/m/d", $update2);
		...
		if ($_POST) {
		    $insert1 = strtotime($_POST['dealine_all']);
            $dealine1=  date("Y/m/d", $insert1);

            $insert2 = strtotime($_POST['dealine_billing']);
			...
			$insert = $wpdb->insert(
	            'dealine_app',
	            array(
	                'dealine_all' => $dealine1,
	                'dealine_billing' => $dealine2,
...
function fc_list_amenity_goods_admin(){
    ...
    // );
      ...
      if (empty($year)) {
          ...
          $shop_togashi = $wpdb->get_results(
              "SELECT * FROM shop_togashi WHERE date_dk LIKE '".$year."%' ORDER BY date_dk DESC"
          );
      }
...
function add_search_year_admin_bar() {
    ...
    $query = "";
    if (isset($current_page) && ($current_page == 'exhibitor-list-admin' || $current_page == 'amenity-goods' || $current_page == 'news' || $current_page == 'dashboard')) {
    ...
    if (isset($_GET['year'])) {
      $current_year = $_GET['year'];
        ...
        switch ($current_page) {
            case 'exhibitor-list-admin':
                $query = "SELECT YEAR(date_dk)
                    FROM register_togashi
                    INNER JOIN register_togashi_link ON register_togashi.id = register_togashi_link.togashi_id
                    WHERE register_togashi_link.catalog_id = ".$location." AND register_togashi_link.status = 1
                    ORDER BY date_dk DESC";
                $link = admin_url('admin.php?page=exhibitor-list-admin');
                break;
            case 'amenity-goods':
                $query = "SELECT YEAR(date_dk) FROM shop_togashi ORDER BY date_dk DESC";
                $link = admin_url('admin.php?page=amenity-goods');
                break;
            case 'news':
                if (isset($post_status)) {
                    $query = "SELECT YEAR(post_date) FROM wp_posts WHERE post_type = 'news' AND post_status = '".$post_status."' ORDER BY post_date DESC";
                    $link = admin_url('edit.php?post_status='.$post_status.'&post_type=news');
                } else {
                    $query = "SELECT YEAR(post_date) FROM wp_posts WHERE post_type = 'news' ORDER BY post_date DESC";
            ...
            case 'dashboard':
                $query = "SELECT YEAR(date_dk)
                    FROM register_togashi
                    INNER JOIN register_togashi_link ON register_togashi.id = register_togashi_link.togashi_id
                    WHERE register_togashi_link.catalog_id = ".$location." ORDER BY date_dk DESC";
        ...
        $wp_admin_bar->add_node( array(
            'id' => 'search_year',
            'title' => __( '<select onchange="location = this.value;"><option value='.$link.'>年別</option>'),
        ) );
        // Get list year
        if ($query) {
            global $wpdb;
