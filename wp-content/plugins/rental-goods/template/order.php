                                    ...
                                    ?>
                                        <tr class="group_spec">
                                            ...
                                            <td class="padding_15 gia_sp">
                                                ...
                                                <input type="hidden" name="product_price<?php echo $item->id; ?>" value="<?php echo $item->item_price; ?>" />
                                            </td>
                                            <td class="padding_15 nhap_sl">
                                                <?php
                                                    // Get cart from session
                                                    ...
                                                    {
                                                        if (isset($_SESSION["cart_products"][$location][$key])
                                                        && key_exists($item->id, $_SESSION["cart_products"][$location][$key]["product_op"])) {
                                                            $qty = $_SESSION["cart_products"][$location][$key]["product_op"][$item->id]['product_qty'];
                                                        }
                                                    }
                                                ?>
                                                <span>申込数</span><br>
                                                <input type="number" class="product_qty" name="product_qty[<?php echo $item->id; ?>]" value="<?php echo $qty; ?>" min="1" max="<?php echo $item->item_number?>">
                                                <input type="hidden" name="qty_stock<?php echo $item->id; ?>" value="<?php echo $item->item_number; ?>">
                                            </td>
                                            <td class="padding_15 add_sp">
                                                <input type="hidden" name="group_product_id<?php echo $item->id; ?>" value="<?php echo $key; ?>" />
                                                <input type="hidden" name="product_ids[]" value="<?php echo $item->id; ?>" />
                                                <button type="submit" class="add_product" name="add_product" value="<?php echo $item->id; ?>" >申し込む</button>
                                            </td>
                                        </tr>
                                    <?php
                                }
